...
function calcularCargaInteligente(inicio, fim) {
  ...
  return `${String(Math.floor(liquido / 60)).padStart(2, "0")}h${String(
    liquido % 60
  ).padStart(2, "0")}m`;
}
...
document
  ...
  .addEventListener("submit", async (e) => {
    ...
    const getSelecionados = (classe) =>
      Array.from(document.querySelectorAll(classe + ":checked")).map(
        (cb) => cb.value
      );
...
function limparFormEscala() {
  ...
  document.getElementById("edit-id-escala").value = "";
  document.getElementById("btn-save-escala").innerText = "Salvar Escala";
}
