...
async function carregarCacheFuncionarios() {
  try {
    ...
    cacheFuncionariosCompleto = [];
    snap.forEach((doc) =>
      cacheFuncionariosCompleto.push({ id: doc.id, ...doc.data() })
    );
  } catch (e) {
    console.error(e);
  }
...
function popularFuncionarios() {
  ...
  const setGlobal = document.getElementById("global-setor").value;
  select.innerHTML = '<option value="">Selecione o funcionário...</option>';
  const filtrados = cacheFuncionariosCompleto.filter(
    (f) =>
      (empGlobal === "TODAS" || f.empresa === empGlobal) &&
      (setGlobal === "TODOS" || f.setor === setGlobal)
  );
  filtrados
    .sort((a, b) => a.nome.localeCompare(b.nome))
    .forEach((f) => {
      select.innerHTML += `<option value="${f.nome}">${f.nome} (${f.empresa})</option>`;
    });
}
...
document
  ...
  .addEventListener("submit", async (e) => {
    ...
    const dados = {
      ...
      status: document.querySelector('input[name="usuario-status"]:checked')
        .value,
      perfilMaster: document.getElementById("usuario-master").checked,
      permissoes: Array.from(
        document.querySelectorAll(".chk-permissao input:checked")
      ).map((cb) => cb.value),
...
async function renderizarUsuarios() {
  ...
  try {
    ...
    snap.forEach((doc) => {
      const u = doc.data();
      const dadosF = cacheFuncionariosCompleto.find(
        (f) => f.nome === u.nomeCompleto
      );
      let passa = true;
      if (empGlobal !== "TODAS" && (!dadosF || dadosF.empresa !== empGlobal))
        passa = false;
      if (setGlobal !== "TODOS" && (!dadosF || dadosF.setor !== setGlobal))
        passa = false;
      if (perLocal !== "TODOS" && (!dadosF || dadosF.periodo !== perLocal))
        passa = false;
      if (passa) {
        container.innerHTML += `<div class="card-usuario"><div class="card-usuario-header"><h3>${
          u.nomeCompleto
        }</h3>${
          u.perfilMaster ? '<span class="badge-master">MASTER</span>' : ""
        }</div><div class="card-usuario-body">${
          dadosF ? `<small>${dadosF.empresa} - ${dadosF.periodo}</small>` : ""
        }<div><b>Login:</b> ${u.login}</div><div class="status-badge-inline ${
          u.status === "Ativo" ? "status-user-ativo" : "status-user-bloqueado"
        }">${u.status}</div><div class="permissoes-list-card">${(
          u.permissoes || []
        )
          .map((p) => `<span class="tag-permissao">${p}</span>`)
          .join(
            ""
          )}</div></div><div class="card-usuario-footer"><i class="fa-solid fa-user-pen" onclick="editarUsuario('${
          doc.id
        }')"></i><i class="fa-solid fa-trash-can" onclick="excluirUsuario('${
          doc.id
        }')"></i></div></div>`;
      }
    });
...
async function editarUsuario(id) {
  ...
  const u = doc.data();
  const select = document.getElementById("select-funcionario-usuario");
  if (!Array.from(select.options).some((o) => o.value === u.nomeCompleto))
    select.innerHTML += `<option value="${u.nomeCompleto}">${u.nomeCompleto}</option>`;
  document.getElementById("select-funcionario-usuario").value = u.nomeCompleto;
  document.getElementById("usuario-login").value = u.login;
  document.getElementById("usuario-senha").value = u.senha;
  document.getElementById("usuario-master").checked = u.perfilMaster || false;
  document.querySelector(
    `input[name="usuario-status"][value="${u.status}"]`
  ).checked = true;
...
function limparFormUsuario() {
  ...
  document
    .querySelectorAll(".chk-permissao input")
