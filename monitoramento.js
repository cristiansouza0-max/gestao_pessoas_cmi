  ...
  for (let i = 0; i < files.length; i++) {
    ...
    json.forEach((row) => {
      ...
      if (segmento && linha && horaBruta !== undefined) {
        let horaNum =
          typeof horaBruta === "number"
            ? Math.floor(horaBruta * 24)
            : parseInt(horaBruta.split(":")[0]);
...
function exibirPrevia(mapa) {
  ...
  for (let chave in mapa) {
    ...
    card.innerHTML = `<b>${obterPrefixo(
      linha
    )}</b> (${empresa})<br>${periodo.toUpperCase()}: ${mapa[chave]} viagens`;
    resumoGrid.appendChild(card);
  }
  areaPrevia.style.display =
    tripsCalculadasGlobal.length > 0 ? "block" : "none";
}
...
async function carregarTabelas() {
  ...
  const snap = await db
    .collection("monitoramento_viagens")
    .orderBy("linha")
    .get();
  corpoManha.innerHTML = "";
  corpoTarde.innerHTML = "";
  ...
  snap.forEach((doc) => {
    ...
    tr.innerHTML = `<td class="td-linhas">${obterPrefixo(d.linha)} <span>${
      d.empresa
    } | ${d.segmento}</span></td><td>${d.uteis || "-"}</td><td>${
      d.sabados || "-"
    }</td><td>${d.domingos || "-"}</td><td>${d.feriados || "-"}</td>`;
    if (d.periodo === "manha") corpoManha.appendChild(tr);
    else corpoTarde.appendChild(tr);
  });
  popularFiltrosSuspensos(
    Array.from(linhasUnicas).sort(),
    Array.from(segmentosUnicos).sort()
  );
}

function popularFiltrosSuspensos(linhas, segmentos) {
  document.getElementById("search-linha").innerHTML =
    '<option value="">Todas as Linhas</option>' +
    linhas.map((l) => `<option value="${l}">${l}</option>`).join("");
  document.getElementById("search-segmento").innerHTML =
