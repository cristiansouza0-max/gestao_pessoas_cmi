...
async function carregarFuncionarios() {
  ...
  try {
    ...
    if (!isMaster) {
      const meuFunc = cacheFuncionarios.find(
        (f) => f.nome === usuarioLogado.nomeCompleto
      );
      if (meuFunc) {
...
function gerarOpcoesVencimento(apelido) {
  ...
  let rawData =
    ...
    funcionario.dataAdmissao_formatada;
  ...
  unsubscribeAusencias = db
    ...
    .onSnapshot((snapshot) => {
      ...
      snapshot.forEach((doc) => {
        ...
        const id = doc.id;
        let statusAtual =
          aus.status === "Aprovado" ? "Aprovada" : aus.status || "Aprovada";
        ...
        if (isMaster) {
          const dF = cacheFuncionarios.find(
            (f) => f.apelido === aus.funcionario
          );
          if (empGlobal !== "TODAS" && (!dF || dF.empresa !== empGlobal))
            passaFiltro = false;
          if (setorGlobal !== "TODOS" && (!dF || dF.setor !== setorGlobal))
            passaFiltro = false;
          if (fFunc !== "TODOS" && aus.funcionario !== fFunc)
            passaFiltro = false;
        }
        ...
        if (passaFiltro) {
          ...
          if (target) {
            ...
            if (aus.tipo === "Férias" && aus.vencimentoReferencia) {
              infoTexto =
                `<strong style="display:block; color:#2980b9; margin-bottom:4px; font-size:0.75rem;">Referente ao Vencimento ${aus.vencimentoReferencia}</strong>` +
                infoTexto;
            }
            ...
            let acoes = isMaster
              ? isPendente
                ? `<button onclick="decidirAusencia('${id}', 'Aprovada', '${aus.funcionario}')" class="btn-aprovar"><i class="fa-solid fa-circle-check"></i></button><button onclick="decidirAusencia('${id}', 'Recusada', '${aus.funcionario}')" class="btn-reprovar"><i class="fa-solid fa-circle-xmark"></i></button>`
            ...
            card.innerHTML = `<div class="card-header-func">${
              aus.funcionario
            } <span class="badge-status-ausencia ${
              isPendente ? "status-pendente" : "status-aprovada"
            }">${statusAtual}</span> <span class="badge-dias">${total} d</span></div><div class="card-body-ausencia"><div class="ausencia-info">${infoTexto}</div><div class="card-action-column">${acoes}</div></div>`;
      ...
      Object.keys(containers).forEach((k) => {
        if (containers[k] && containers[k].innerHTML === "")
          containers[k].innerHTML =
            "<p style='font-size:0.7rem; color:gray; text-align:center;'>Vazio</p>";
      });
    });
}

document
  .getElementById("form-ausencia")
      ...
      .onSnapshot((snap) => {
        ...
        if (nomes.length > 0)
          exibirNotificacaoCentral({
            ...
            mensagem: `Pedido(s) de: <strong>${[...new Set(nomes)].join(
              ", "
            )}</strong>`
          });
      });
        ...
        snap.forEach((doc) => {
          ...
          if (aus.status === "Aprovada") {
            exibirNotificacaoCentral({
              ...
              mensagem: `Sua ausência de ${aus.tipo} foi aprovada.`
            });
            db.collection("ausencias")
              .doc(doc.id)
              .update({ notificadoUser: true });
          } else if (aus.status === "Recusada") {
            exibirNotificacaoCentral({
              titulo: "Recusada",
              cor: "#e74c3c",
              icone: "fa-solid fa-circle-xmark",
              mensagem: `Sua ausência foi recusada: ${aus.justificativa}`
            });
            db.collection("ausencias")
              .doc(doc.id)
              .update({ notificadoUser: true });
...
async function decidirAusencia(id, decisao, nomeFunc) {
  if (decisao === "Aprovada")
    await db
      .collection("ausencias")
      .doc(id)
      .update({ status: "Aprovada", notificadoUser: false });
  else {
    const m = prompt("Motivo da recusa:");
    if (m)
      await db
        .collection("ausencias")
        .doc(id)
...
function editarAusencia(id) {
  ...
  if (aus.tipo === "Férias") {
    document.getElementById("vencimento-ferias").value =
      aus.vencimentoReferencia || "";
  }
...
async function excluirAusencia(id) {
  if (confirm("Excluir?")) await db.collection("ausencias").doc(id).delete();
}

function calcularTotalDias(str, modo) {
  if (!str) return 0;
  if (modo === "single") return 1;
  if (modo === "multiple") return str.split(" ; ").length;
  const p = str.split(" até ");
