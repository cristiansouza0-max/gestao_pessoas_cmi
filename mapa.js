...
const feriadosBase = [
  ...
  },
  {
    dia: 30,
    mes: 11,
    nome: "Emancipação Franco da Rocha",
    tipo: "municipal",
    empresa: "VCCL"
  },
  {
...
function processarDatas(reg) {
  ...
  if (reg.modo === "range") {
    ...
    reg.datas.split(" ; ").forEach((s) => {
      if (s) dates.push(parse(s));
    });
  }
  return dates;
}
...
function getDiasAusencia(apelido, lista, tipo, m, y) {
  ...
  lista
    .filter((a) => a.funcionario === apelido && tiposBusca.includes(a.tipo))
    .forEach((reg) => {
...
function getDetalhesFolgas(apelido, lista, m, y) {
  let folgas = {};
  lista
    .filter((a) => a.funcionario === apelido && a.tipo === "Folga")
    .forEach((reg) => {
      processarDatas(reg).forEach((dt) => {
        if (dt.getMonth() + 1 === m && dt.getFullYear() === y)
          folgas[dt.getDate()] = reg.observacao;
      });
    });
  return folgas;
}
...
function fPediuManualGlobal(apelido, dia, m, y, ausencias) {
  ...
  return ausencias.some(
    (a) =>
      a.funcionario === apelido &&
      a.tipo === "Folga" &&
      (a.observacao === "Pedida" ||
        a.observacao === "Marcada" ||
        a.observacao === "Programada") &&
...
function simularEscalasAnoTodo(ausencias, funcionarios, anoAlvo, regrasAtivas) {
  ...
  const assistenteInterVCCL = funcionarios.find(
    (f) =>
      f.empresa === "VCCL" &&
      f.periodo === "Intermediário" &&
      f.funcao === "Assistente"
  );
  ...
  while (dSim <= dFim) {
    const d = dSim.getDate(),
      m = dSim.getMonth() + 1,
      y = dSim.getFullYear();
    const chave = `${d}-${m}-${y}`,
      sem = dSim.getDay();
    const idxFDSGeral = Math.abs(
      Math.floor(
        (dSim.getTime() - dataBaseRef.getTime()) / (1000 * 60 * 60 * 24 * 7)
      )
    );
    ...
    if (dSim < dataNovaRegraMarço) {
      ...
      if (sem === 6) {
        ...
        const dRefSab = dSim;
        const idxFDS = Math.floor(
          (dRefSab.getTime() - dataMarcoZeroFDSManha.getTime()) /
            (1000 * 60 * 60 * 24 * 7)
        );

        let manual =
          equipeBase.find((n) => fPediuManualGlobal(n, d, m, y, ausencias)) ||
          (fPediuManualGlobal("Milena Benites", d, m, y, ausencias)
            ? "Milena Bennites"
            : null);
        ...
        if (manual) {
          ...
          let escala =
            idxFDS % 2 !== 0
              ? "Milena Benites"
              : equipeBase[pManhaRodizio % equipeBase.length];
        ...
        equipeEspelho.forEach((n) => {
          if (n !== quemFolgouSabadoManha) mManhaAvul[`${chave}-${n}`] = true;
        });
      ...
      if (sem === 6) {
        ...
        let manual = equipeNova.find((n) =>
          fPediuManualGlobal(n, d, m, y, ausencias)
        );
    ...
    // --- DEMAIS LÓGICAS ---
    if (sem === 6) {
      const dDom = new Date(dSim);
      dDom.setDate(dDom.getDate() + 1);
      const ultD = obterUltimoDomingo(y, m);
      if (d !== ultD - 1) {
        if (idxFDSGeral % 2 === 0) {
          mNoite[chave + "-AVUL"] = true;
          mNoite[
            `${dDom.getDate()}-${
              dDom.getMonth() + 1
            }-${dDom.getFullYear()}-VCCL`
          ] = true;
        } else {
          mNoite[chave + "-VCCL"] = true;
          mNoite[
            `${dDom.getDate()}-${
              dDom.getMonth() + 1
            }-${dDom.getFullYear()}-AVUL`
          ] = true;
        }
      }
    }
    ...
    if (regrasAtivas.equipeTarde && sem === 6) {
      ...
      for (let diaA = 1; diaA < d; diaA++) {
        ...
        if (
          dataA.getDay() === 0 &&
          mTardeAvul[`${diaA}-${m}-${y}-Márcia Cristina`]
        ) {
      ...
      const marciaComecaFeriasNaSegunda =
        emFeriasGlobal(
          "Márcia Cristina",
          dSeg.getDate(),
          dSeg.getMonth() + 1,
      ...
      let marciaTrabalhaSabado =
        (ehSabadoAntesUltimoDom && !marciaJaFolgouDomingoEsteMes) ||
      ...
      if (
        !mTardeAvul[`${chaveSab}-Márcia Cristina`] ||
        marciaComecaFeriasNaSegunda
      ) {
    ...
    if (sem === 6 || sem === 0) {
      const m_vccl = funcsDia.filter(
        (f) => f.empresa === "VCCL" && f.periodo === "Manhã"
      );
      const t_vccl = funcsDia.filter(
        (f) => f.empresa === "VCCL" && f.periodo === "Tarde"
      );
      let duplas = [];
      let interOcupado = false;
      if (m_vccl.length === 2)
        duplas.push({ p1: m_vccl[0].apelido, p2: m_vccl[1].apelido });
      else if (m_vccl.length === 1 && interApelido) {
        duplas.push({ p1: m_vccl[0].apelido, p2: interApelido });
        interOcupado = true;
      }
      if (t_vccl.length === 2)
        duplas.push({ p1: t_vccl[0].apelido, p2: t_vccl[1].apelido });
      else if (t_vccl.length === 1 && interApelido) {
        duplas.push({ p1: t_vccl[0].apelido, p2: interApelido });
        interOcupado = true;
      }
      duplas.forEach((dupla) => {
        ...
        if (sem === 6) {
          if (
            fPediuManualGlobal(nOff, d + 1, m, y, ausencias) ||
            precisaSubstituicao(nOff, new Date(y, m - 1, d + 2))
          )
            darF = nOff === tA ? tB : tA;
        } else if (sem === 0) {
          if (
        ...
        if (fPediuManualGlobal(darF === tA ? tB : tA, d, m, y, ausencias))
          darF = null;
        if (darF) mVCCL[`${chave}-${darF}`] = true;
      });
      if (!interOcupado && sem === 0 && interApelido)
        mVCCL[`${chave}-${interApelido}`] = true;
    }
    dSim.setDate(dSim.getDate() + 1);
  }
  return {
    tardeAvul: mTardeAvul,
    manhaAvul: mManhaAvul,
    noite: mNoite,
    vccl: mVCCL
  };
}
...
async function gerarMapa() {
  const mes = parseInt(document.getElementById("mapa-mes").value),
    ano = parseInt(document.getElementById("mapa-ano").value);
  let empSel = isMaster
    ? Array.from(document.querySelectorAll(".filtro-emp:checked")).map(
        (cb) => cb.value
      )
    : ["AVUL", "VCCL", "VSBL"];
  let setSel = isMaster
    ? Array.from(document.querySelectorAll(".filtro-set:checked")).map(
        (cb) => cb.value
      )
    : ["Tráfego", "Monitoramento"];
  const perSel = isMaster
    ? Array.from(document.querySelectorAll(".filtro-per:checked")).map(
        (cb) => cb.value
      )
    : ["Manhã", "Intermediário", "Tarde", "Noite", "Integral"];
  ...
  try {
    ...
    let funcionarios = snapFunc.docs
      .map((d) => {
        const data = d.data();
      ...
      .filter((f) => {
        ...
        return (
          dtAdm <= dataFimMes &&
          (f.status === "Ativo" || (dtDem && dtDem >= dataInicioMes))
        );
      });

    if (!isMaster) {
      const meuFunc = funcionarios.find(
        (f) =>
          f.nome.trim().toLowerCase() ===
          usuarioLogado.nomeCompleto.trim().toLowerCase()
      );
      if (meuFunc) {
        empSel = [meuFunc.empresa];
        setSel = [meuFunc.setor || "Tráfego"];
    ...
    const sim = simularEscalasAnoTodo(
      ausencias,
      funcionarios,
      ano,
      regrasAtivas
    ...
    ["AVUL", "VCCL", "VSBL"].forEach((empresa) => {
      ...
      perSel.forEach((per) => {
        ...
        funcsPeriodo.sort((a, b) => a.nome.localeCompare(b.nome));
        ...
        for (let d = 1; d <= diasNoMes; d++) {
          ...
          tableHtml += `<th>${String(d).padStart(2, "0")}/${String(
            mes
          ).padStart(2, "0")}<br>${diaSem}</th>`;
        }
        ...
        funcsPeriodo.forEach((f) => {
          ...
          const dtAdm = new Date(f.admissao + "T00:00:00");
          const dtDem =
            f.status === "Inativo" && f.demissao
              ? new Date(f.demissao + "T00:00:00")
              : null;
          let ferias = getDiasAusencia(
            f.apelido,
            ausencias,
            "Férias",
            mes,
          ...
          let outrosAfast = getDiasAusencia(
            f.apelido,
            ausencias,
            ["Afastamento", "Licença"],
            mes,
          ...
          for (let d = 1; d <= diasNoMes; d++) {
            const dObj = new Date(ano, mes - 1, d),
              sem = dObj.getDay(),
              chave = `${d}-${mes}-${ano}`;
            const eFer = feriadosBase.some(
              (fer) =>
                fer.dia === d &&
                fer.mes === mes &&
                (fer.tipo !== "municipal" || fer.empresa === empresa)
            );
            if (dObj < dtAdm) {
            ...
            if (dtDem && dObj >= dtDem) {
              tableHtml += `<td colspan="${
                diasNoMes - d + 1
              }" class="dia-demitido">Demitido</td>`;
              d = diasNoMes;
              continue;
            }
            let simboloA = "";
            if (f.nascimento) {
              ...
              if (dN === d && mN === mes)
                simboloA = `<span class="symbol-a">A</span>`;
            }
            ...
            if (outrosAfast.includes(d)) {
              ...
              tableHtml += `<td colspan="${count}" class="dia-afastamento">${simboloA}AF</td>`;
              d += count - 1;
              continue;
            }
            let conteudo = "",
            ...
            if (folgasInfo[d]) {
              conteudo = `<span class="${
                folgasInfo[d] === "Pedida"
                  ? "folga-pedida"
                  : folgasInfo[d] === "Marcada"
                  ? "folga-marcada"
                  : "folga-programada"
              }">${folgasInfo[d] === "Programada" ? "F" : "X"}</span>`;
              decidido = true;
            }
            if (
              !decidido &&
              empresa === "VCCL" &&
              sim.vccl[`${chave}-${f.apelido}`]
            ) {
              conteudo = "<b>X</b>";
              decidido = true;
            }
            ...
            if (!decidido && f.periodo === "Noite") {
              if (sem === 6) {
                if (
                  f.apelido === "Fábio Miguel" &&
                  d === obterUltimoDomingo(ano, mes) - 1
                ) {
                  conteudo = "<b>X</b>";
                  decidido = true;
                } else if (
                  (sim.noite[chave + "-VCCL"] && f.empresa === "VCCL") ||
                  (sim.noite[chave + "-AVUL"] && f.empresa === "AVUL")
                ) {
                  conteudo = "<b>X</b>";
                  decidido = true;
                }
                ...
                if (
                  f.apelido === "Osmair Lopes" &&
                  d === obterUltimoDomingo(ano, mes)
                ) {
                  conteudo = "<b>X</b>";
                  decidido = true;
                } else if (
            ...
            if (!decidido) {
              if (f.funcao === "Líder" && (sem === 0 || sem === 6 || eFer))
                conteudo = "<b>X</b>";
              else if (
                sem === 0 &&
                d < diasNoMes &&
                new Date(ano, mes - 1, d + 1).getDay() === 1 &&
                emFeriasGlobal(f.apelido, d + 1, mes, ano, ausencias)
              )
                conteudo = "<b>X</b>";
            }
            tableHtml += `<td class="${
...
function toggleSimbolosExtras() {
  const container = document.getElementById("mapa-container"),
    chk = document.getElementById("chk-exibir-extras");
  if (chk && container) {
...
async function salvarMapaDefinitivo() {
  const mes = document.getElementById("mapa-mes").value,
    ano = document.getElementById("mapa-ano").value,
    html = document.getElementById("mapa-container").innerHTML;
  try {
    await db
      .collection("mapas_salvos")
      .doc(`${ano}-${mes}`)
      .set({ html, salvoEm: Date.now() });
    alert("Mapa salvo!");
  } catch (e) {
...
function ajustarSidebar() {
  ...
  if (
    ...
    paginaAtual !== "index" &&
    paginaAtual !== "" &&
    !permissoes.includes(paginaAtual)
  )
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  if (!isMaster) {
    document
      .querySelectorAll(".filter-group-visible, .btn-save-blue")
      .forEach((el) => (el.style.display = "none"));
  }
  const inputAno = document.getElementById("mapa-ano"),
    selectMes = document.getElementById("mapa-mes");
  ...
  selectMes.addEventListener("change", gerarMapa);
  document
    .querySelectorAll(".filtro-emp, .filtro-set, .filtro-per")
    .forEach((el) => el.addEventListener("change", gerarMapa));
  gerarMapa();
});
