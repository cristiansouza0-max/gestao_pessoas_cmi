...
function toggleDataStatus() {
  ...
  document.getElementById("col-demissao").style.visibility =
    status === "Inativo" ? "visible" : "hidden";
}
...
function atualizarFiltroJornadas() {
  ...
  const filtradas = todasJornadasCache.filter(
    (j) =>
...
document
  .getElementById("form-func")
  .addEventListener("submit", async function (e) {
    e.preventDefault();
    const id = document.getElementById("edit-id").value;
    const jornadasIds = Array.from(
      document.querySelectorAll(".chk-jornada:checked")
    ).map((cb) => cb.value);
  ...
  snap.forEach((doc) => {
    ...
    if (f.jornadasIds) {
      f.jornadasIds.forEach((jid) => {
        ...
        if (j)
          jornadasHtml += `<div class="tag-escala-card">${j.periodo} - Jornada ${j.ordem}</div>`;
      });
    }
    ...
    card.innerHTML = `
            ...
            <div class="card-body-container">
                <div class="card-info-content">
                    ...
                    <div><span>Apelido:</span> ${
                      f.apelido
                    } - <span>Registro:</span> ${f.registro}</div>
                    <div><span>Data de Nascimento:</span> ${formatarData(
                      f.nascimento
                    )}</div>
                    <div><span>Período:</span> ${
                      f.periodo
                    } - <span>Função:</span> ${f.funcao}</div>
                    <div class="container-escalas-card">
                        <span>Jornada:</span> ${jornadasHtml || "Não associada"}
                    </div>
                </div>
                <div class="card-action-column">
                    <button onclick="editarFunc('${
                      doc.id
                    }')" class="btn-icon-edit"><i class="fa-solid fa-pencil"></i></button>
                    <button onclick="excluirFunc('${
                      doc.id
                    }')" class="btn-icon-delete"><i class="fa-solid fa-trash-can"></i></button>
                </div>
            </div>`;
