...
async function gerarEscalaEquipe() {
  ...
  const empSel = isMaster
    ? Array.from(document.querySelectorAll(".filtro-emp-jor:checked")).map(
        (cb) => cb.value
      )
    : [];
  const setSel = isMaster
    ? Array.from(document.querySelectorAll(".filtro-set-jor:checked")).map(
        (cb) => cb.value
      )
    : [];
  const perSel = isMaster
    ? Array.from(document.querySelectorAll(".filtro-per-jor:checked")).map(
        (cb) => cb.value
      )
    : [];
  ...
  try {
    const [
      ...
      db
        .collection("parametros_feriados")
        .doc(`${ano}-${mes + 1}`)
        .get()
    ]);
    ...
    if (cabecalhoMaster) {
      cabecalhoMaster.innerText = isMaster
        ? `ESCALA DE TRABALHO - EQUIPE - ${mesesNomesLongos[mes]} ${ano}`
        : `ESCALA DE TRABALHO - ${usuarioLogado.nomeCompleto} - ${mesesNomesLongos[mes]} ${ano}`;
    }

    const sim = simularEscalasSincronizadas(
      todasAusencias,
      listaFuncs,
      ano,
      regrasAtivas,
    ...
    listaFuncs
      ...
      .forEach((f) => {
        ...
        const jors = jornadas.filter(
          (j) => f.jornadasIds && f.jornadasIds.includes(j.id)
        );
        const jPrincipal = jors.find((j) => j.ordem === 1) || jors[0];

        let totalMinutos = 0,
          rowsHtml = "",
          jaProcessada = new Set();

        for (let d = 1; d <= diasNoMes; d++) {
          const dataAtu = new Date(ano, mes, d),
            sem = dataAtu.getDay(),
            chave = `${d}-${mes + 1}-${ano}`;
          const dataFmt = `${String(d).padStart(2, "0")}/${String(
            mes + 1
          ).padStart(2, "0")}`;
          const diaSemFmt = ["DOM", "SEG", "TER", "QUA", "QUI", "SEX", "SÁB"][
            sem
          ];
          const eFeriado = feriadosParam[`${f.empresa}-${d}-${f.periodo}`] > 0;
          let classeRow = eFeriado
            ? "row-feriado"
            : sem === 6
            ? "row-sabado"
            : sem === 0
          ...
          const ausBloco = todasAusencias.find(
            (a) =>
              a.funcionario === f.apelido &&
              ["Férias", "Licença", "Afastamento", "Falta"].includes(a.tipo) &&
              isDataNoRange(dataAtu, a)
          );

          if (ausBloco) {
            let classeCor =
              ausBloco.tipo === "Férias"
                ? "ferias"
                : ausBloco.tipo === "Falta"
                ? "falta"
                : "outros";
            ...
            const ausFolga = todasAusencias.find(
              (a) =>
                a.funcionario === f.apelido &&
                a.tipo === "Folga" &&
                isDataNoRange(dataAtu, a)
            ...
            if (ausFolga || folgaEscala) {
              ...
              let idsEscala = jPrincipal
                ? eFeriado
                  ? jPrincipal.escalas.feriado
                  : sem === 0
                  ? jPrincipal.escalas.domingo
                  : sem === 6
                  ? jPrincipal.escalas.sabado
                  : jPrincipal.escalas.uteis
                : [];
              const arrayIds = Array.isArray(idsEscala)
                ? idsEscala
                : idsEscala
                ? [idsEscala]
                : [];
              ...
              if (arrayIds.length > 0) {
                ...
                arrayIds.forEach((id) => {
                  ...
                  if (esc) {
                    ...
                    txtEsc +=
                      (txtEsc ? "<br>" : "") +
                      `${esc.inicioJornada}-${esc.fimJornada}`;
                  }
                });
                totalMinutos += minDia;
                const hD = Math.floor(minDia / 60),
                  mD = minDia % 60;
                rowsHtml += `<tr class="${classeRow}"><td>${dataFmt}</td><td>${diaSemFmt}</td><td colspan="2">${txtEsc}</td><td>${String(
                  hD
                ).padStart(2, "0")}:${String(mD).padStart(2, "0")}</td></tr>`;
              } else {
        ...
        const hT = Math.floor(totalMinutos / 60),
          mT = totalMinutos % 60;
        wrapper.innerHTML = `<div class="box-nome-func">${
          f.apelido
        }</div><table class="tabela-jornada"><thead><tr><th>Data</th><th>Dia</th><th colspan="2">Horários</th><th>Tot.</th></tr></thead><tbody>${rowsHtml}</tbody><tfoot><tr><td colspan="4" class="footer-total">TOTAL</td><td class="footer-total">${String(
          hT
        ).padStart(2, "0")}:${String(mT).padStart(
          2,
          "0"
        )}</td></tr></tfoot></table>`;
...
function simularEscalasSincronizadas(
  ausencias,
  funcionarios,
  anoAlvo,
  regrasAtivas,
...
function isDataNoRange(data, reg) {
  ...
  if (reg.modo === "range") {
    const [i, f] = reg.datas.split(" até ");
    return data >= toD(i) && data <= toD(f);
  }
  return reg.datas
    .split(" ; ")
    .some((d) => toD(d).toDateString() === data.toDateString());
}
...
function fPediuManualGlobal(apelido, dia, m, y, ausencias) {
  ...
  return ausencias.some(
    (a) =>
      ...
      a.tipo === "Folga" &&
      ["Pedida", "Marcada", "Programada"].includes(a.observacao) &&
      isDataNoRange(new Date(y, m - 1, dia), a)
  );
}
...
function calcularHoras(esc) {
  ...
  let ent = toM(esc.inicioJornada),
    sai = toM(esc.fimJornada);
  if (sai < ent) sai += 1440;
...
function ajustarSidebar() {
  ...
  document.querySelectorAll(".sidebar ul li a").forEach((link) => {
    ...
    if (link.getAttribute("href") === "#" || h === "index") {
      link.parentElement.style.display = "block";
      return;
    }
    if (!isMaster && !p.includes(h)) link.parentElement.style.display = "none";
