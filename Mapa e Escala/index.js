...
async function carregarDashboard() {
  ...
  try {
    ...
    snapFunc.forEach((doc) =>
      cacheFuncionarios.push({ id: doc.id, ...doc.data() })
    );
    cacheAusencias = [];
    snapAus.forEach((doc) => cacheAusencias.push(doc.data()));
    ...
    if (isMaster) {
      document.getElementById("total-func").innerText = funcsAtivos.length;
      document.getElementById("total-aux").innerText = funcsAtivos.filter(
        (f) => f.funcao === "Auxiliar"
      ).length;
      document.getElementById("total-ast").innerText = funcsAtivos.filter(
        (f) => f.funcao === "Assistente"
      ).length;
      document.getElementById("total-lid").innerText = funcsAtivos.filter(
        (f) => f.funcao === "Líder"
      ).length;
      document.getElementById("total-int").innerText = funcsAtivos.filter(
        (f) => f.periodo === "Intermediário"
      ).length;
      document.getElementById("total-apr").innerText = funcsAtivos.filter(
        (f) => f.funcao === "Aprendiz"
      ).length;
    }
  } catch (e) {
...
function renderizarResumos(lista, hoje) {
  const mesA = hoje.getMonth() + 1;
  const v = lista.filter(
    (f) => f.nascimento && parseInt(f.nascimento.split("-")[1]) === mesA
  );
  const e = lista.filter(
    (f) => f.admissao && parseInt(f.admissao.split("-")[1]) === mesA
  );
  document.getElementById("lista-aniversario-vida").innerHTML = v.length
    ? v
        .map(
          (f) =>
            `<li class="anniversary-item"><span>${
              f.apelido
            }</span><span class="anniversary-date">Dia ${
              f.nascimento.split("-")[2]
            }</span></li>`
        )
        .join("")
    : '<li class="empty-info">Sem aniversariantes</li>';
  document.getElementById("lista-aniversario-empresa").innerHTML = e.length
    ? e
        .map(
          (f) =>
            `<li class="anniversary-item"><span>${f.apelido} - Dia ${
              f.admissao.split("-")[2]
            }</span><span class="anniversary-date">${
              hoje.getFullYear() - parseInt(f.admissao.split("-")[0])
            } anos</span></li>`
        )
        .join("")
    : '<li class="empty-info">Sem registros</li>';
}
...
function processarResumosAusencia(ausencias, dataHoje) {
  ...
  ausencias.forEach((a) => {
    ...
    dts.forEach((d) => {
      ...
      if (
        a.tipo === "Folga" &&
        d.getMonth() === mesProximo &&
        d.getFullYear() === anoProximo
      ) {
        if (!folgasData[a.funcionario])
          folgasData[a.funcionario] = {
...
async function renderizarTimelineFerias() {
    ...
    lista = lista.filter(f => {
        ...
        // Se admitido em 19/03/2025, o vencimento 18/03/2025 é ANTES da admissão, portanto inválido.
...
=======
  document.getElementById("lista-faltas-mes").innerHTML =
    Object.entries(faltasData)
      .map(
        ([nome, qtd]) =>
          `<li class="summary-item"><span>${nome}</span> <span class="qty">${qtd} dias</span></li>`
      )
      .join("") || '<li class="empty-info">Nenhuma falta</li>';
  document.getElementById("lista-folgas-proximo").innerHTML =
    Object.entries(folgasData)
      .map(
        ([nome, info]) =>
          `<li class="summary-item"><span>${nome} ${info.texto}</span> <span class="qty">${info.qtd} d</span></li>`
      )
...
async function renderizarTimelineFerias() {
  ...
  const anoFiltro = parseInt(
    document.getElementById("filtro-ano-timeline").value
  );
  const mesValor = document.getElementById("filtro-mes-timeline").value;
  ...
  let lista = cacheFuncionarios.filter(
    (f) => f.status !== "Inativo" && f.funcao !== "Aprendiz"
  );
  ...
  if (!isMaster)
    ...
    if (fPeriodo !== "TODOS")
      lista = lista.filter((f) => f.periodo === fPeriodo);
    if (fFunc !== "TODOS") lista = lista.filter((f) => f.apelido === fFunc);
  }
  ...
  header.innerHTML =
    '<th class="col-fixa">Funcionários</th><th class="col-vencimento">Vencimento</th>';

  dias.forEach((d) => {
    const diaNome = diasSemana[d.getDay()];
    // Adiciona a Data e o Dia da Semana abaixo
    header.innerHTML += `<th>${String(d.getDate()).padStart(2, "0")}/${String(
      d.getMonth() + 1
    ).padStart(
      2,
      "0"
    )}<br><span style="font-size: 0.55rem; opacity: 0.8;">${diaNome}</span></th>`;
  });

  corpo.innerHTML = "";
  lista.forEach((f) => {
    const vencObj = calcularVencimentoNoAno(f.admissao, anoFiltro);
    const vencFmt = vencObj
      ? vencObj.toLocaleDateString("pt-BR", {
          day: "2-digit",
          month: "2-digit",
          year: "2-digit"
        })
    ...
    for (let i = 0; i < dias.length; i++) {
      const dStr = dias[i].toDateString();
      const aus = cacheAusencias.find(
        (a) =>
          a.funcionario === f.apelido &&
          a.tipo === "Férias" &&
          parseDatas(a).some((dt) => dt.toDateString() === dStr)
      );

      if (aus) {
        const datesInAus = parseDatas(aus).filter(
          (dt) => dt >= dias[i] && dt <= dataFimTabela
        );
        const span = datesInAus.length;
        const obsLimpa = (aus.observacao || "").toLowerCase().trim();
        ...
        let corBarra = "#9b59b6";
        let textoBarra =
          obsLimpa === "programada" ? "Férias Programada" : "Férias Marcada";

        // --- NOVA LÓGICA DE TEXTO DINÂMICO ---
        const mesFiltroAtivo =
          document.getElementById("filtro-mes-timeline").value !== "TODOS";
        ...
        if (aus.vencimentoReferencia) {
          if (mesFiltroAtivo && span > 10) {
            textoBarra += ` referente ao vencimento de ${aus.vencimentoReferencia}`;
          } else if (!mesFiltroAtivo && span > 10) {
            textoBarra += ` referente a  ${aus.vencimentoReferencia}`;
          }
>>>>>>> 95da380d21748e1ffcf794b66cd376a44bbd3e4b
        }
        ...
        tr += `<td colspan="${span}" 
                           title="${textoBarra}"
                           style="background:${corBarra}; color:white; font-size:12px; font-weight:bold; text-align:center; white-space: nowrap; border: 1px solid black; overflow: hidden; text-overflow: ellipsis; padding: 0 4px;">
                           ${textoBarra}
                       </td>`;
...
function popularFiltroFuncTimeline(lista) {
  ...
  lista
    ...
    .forEach((f) => {
      s.innerHTML += `<option value="${f.apelido}" ${
        f.apelido === atual ? "selected" : ""
      }>${f.apelido}</option>`;
    });
}
...
function configurarTitulosDinamicos(data) {
  ...
  document.getElementById(
    "titulo-faltas-mes"
  ).innerText = `Faltas do Mês (${format(0)})`;
  document.getElementById(
    "titulo-folgas-proximo"
  ).innerText = `Próximas Folgas (${format(1)})`;
}
...
async function renderizarAprendizesDashboard(empFiltro, setorFiltro) {
  const hojeObj = new Date();
  const hojeD = ["Dom", "Seg", "Ter", "Qua", "Qui", "Sex", "Sáb"][
    hojeObj.getDay()
  ];
  const el = document.getElementById("lista-aprendizes-hoje");
  if (!el) return;
  try {
    ...
    const confs = {};
    sConf.forEach((d) => (confs[d.id] = d.data()));
    const escs = {};
    sEsc.forEach((d) => (escs[d.id] = d.data()));
    let lista = cacheFuncionarios.filter(
      (f) => f.funcao === "Aprendiz" && f.status === "Ativo"
    );
    if (empFiltro !== "TODAS")
      lista = lista.filter((f) => f.empresa === empFiltro);
    ...
    let html = lista
      .map((f) => {
        ...
        if (c && c.dias.includes(hojeD)) {
          const e = escs[c.escalaId];
          return `<li class="anniversary-item"><span>${f.apelido} <small>(${
            f.empresa
          })</small></span><span class="anniversary-date">${
            e ? e.inicioJornada : "00:00"
          }</span></li>`;
        }
        return "";
      })
