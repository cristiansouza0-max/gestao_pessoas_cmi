  ...
  feriadosBase
    ...
    .forEach((f) => {
      ["AVUL", "VCCL", "VSBL"].forEach((empresa) => {
        ...
        periodosNomes.forEach((p) => {
          const idRef = `${empresa}-${f.dia}-${p}`;
          inputs += `<div class="periodo-input-box"><label>${p.substring(
            0,
            3
          )}</label><input type="number" class="input-feriado-val" data-ref="${idRef}" value="${
            dados[idRef] || 0
          }"></div>`;
>>>>>>> 95da380d21748e1ffcf794b66cd376a44bbd3e4b
        });
        card.innerHTML = `<div class="feriado-header"><span>${f.dia
          .toString()
          .padStart(2, "0")}/${mes.toString().padStart(2, "0")} - ${diaSem} - ${
          f.nome
        }</span></div><div class="grid-periodos">${inputs}</div>`;
        if (containers[empresa]) containers[empresa].appendChild(card);
      });
    });
}
...
async function carregarRegrasEspeciais() {
    ...
    if (doc.exists) {
        const d = doc.data();
        if (document.getElementById('regra-equipe-manha')) document.getElementById('regra-equipe-manha').checked = d.equipeManha || false;
        if (document.getElementById('regra-equipe-noite')) document.getElementById('regra-equipe-noite').checked = d.equipeNoite || false;
        if (document.getElementById('regra-equipe-tarde')) document.getElementById('regra-equipe-tarde').checked = d.equipeTarde || false;
    }
}
...
=======
  ...
  const ano = document.getElementById("filtro-ano-param").value;
  const dados = {
    folgasDoMes: document.getElementById("folgas-mes-param").value
  };
...
async function carregarRegrasEspeciais() {
  ...
  if (doc.exists) {
    ...
    if (document.getElementById("regra-equipe-manha"))
      document.getElementById("regra-equipe-manha").checked =
        d.equipeManha || false;

    if (document.getElementById("regra-equipe-noite"))
      document.getElementById("regra-equipe-noite").checked =
        d.equipeNoite || false;

    if (document.getElementById("regra-equipe-tarde"))
      document.getElementById("regra-equipe-tarde").checked =
        d.equipeTarde || false;
  }
}
...
async function salvarRegraEspecial(regra) {
  ...
  switch (regra) {
    case "equipeManha":
      idElemento = "regra-equipe-manha";
      break;
    case "equipeNoite":
      idElemento = "regra-equipe-noite";
      break;
    case "equipeTarde":
      idElemento = "regra-equipe-tarde";
      break;
  }
...
=======
  const escalas = [];
  snapEscalas.forEach((doc) => escalas.push({ id: doc.id, ...doc.data() }));
  const configs = {};
  snapConfigs.forEach((doc) => (configs[doc.id] = doc.data()));
...
>>>>>>> 95da380d21748e1ffcf794b66cd376a44bbd3e4b
    ...
    const card = document.createElement("div");
    card.className = "card-aprendiz";
    let flags = diasSemanaArr
      .map(
        (d) =>
          `<label class="flag-dia">${d}<input type="checkbox" class="chk-${fId}" value="${d}" ${
            c.dias.includes(d) ? "checked" : ""
          }></label>`
      )
      .join("");
    let options = escalas
...
=======
                <h4>${f.nome} ${
      f.status === "Inativo" ? '<small style="color:red">(INATIVO)</small>' : ""
    }</h4>
>>>>>>> 95da380d21748e1ffcf794b66cd376a44bbd3e4b
...
async function salvarAprendiz(id) {
  const dias = Array.from(document.querySelectorAll(`.chk-${id}:checked`)).map(
    (cb) => cb.value
  );
  const escId = document.getElementById(`sel-escala-${id}`).value;
  await db
    .collection("config_aprendizes")
    .doc(id)
    .set({ dias, escalaId: escId });
...
=======
  ...
  for (let dia = 1; dia <= uDia; dia++) {
    ...
    if (new Date(ano, mes - 1, dia).getDay() === 0)
      d.classList.add("day-sunday");
    if (feriadosBase.some((f) => f.dia === dia && f.mes === mes))
      d.classList.add("day-holiday");
    grid.appendChild(d);
  }
  document.getElementById("modal-calendario").style.display = "block";
>>>>>>> 95da380d21748e1ffcf794b66cd376a44bbd3e4b
...
window.onclick = function (e) {
  if (e.target == document.getElementById("modal-calendario")) fecharModal();
};
