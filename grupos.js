...
async function distribuirLinhasParaGrupos() {
  ...
  try {
    ...
    if (linhasAlvo.length === 0) {
      alert("Nenhum dado encontrado.");
      return;
    }
    linhasAlvo.sort(
      (a, b) => (parseInt(b[diaCampo]) || 0) - (parseInt(a[diaCampo]) || 0)
    );
    ...
    linhasAlvo.forEach((linha) => {
      ...
      grupos[0].linhas.push({
        prefixo: obterPrefixo(linha.linha),
        qtd: qtd,
    ...
    const diaTexto = document.querySelector(
      `#filt-dia option[value="${diaCampo}"]`
    ).text;
...
async function carregarGruposExistentes() {
  ...
  try {
    ...
    if (snap.empty) {
      container.innerHTML =
        "<p style='text-align: center; color: #999; padding: 40px;'>Nenhum histórico encontrado.</p>";
      return;
    }
...
function imprimirSelecionados() {
  const selecionados = Array.from(
    document.querySelectorAll(".check-bloco:checked")
  ).map((cb) => cb.value);
  if (selecionados.length === 0) {
    alert("Selecione ao menos um bloco para imprimir.");
    return;
  }
